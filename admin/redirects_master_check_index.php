...
if(isset($_REQUEST['Reindex']) && $_REQUEST['Reindex']=='Y') {
    ...
    if(!isset($site['SERVER_NAME']) || !$site['SERVER_NAME']) {
        $message = new CAdminMessage(array(
            'MESSAGE' => GetMessage('SEO2_REINDEX_NO_SITE'),
            'DETAILS' => GetMessage('SEO2_REINDEX_NO_SITE_DESC', array('#SITE_ID#'=>$site['LID'])),
        	'TYPE' => 'ERROR',
        	'HTML' => true
        ));
        echo $message->Show();
    ...
    $message = new CAdminMessage(array(
            'MESSAGE' => ($endOfResults)? GetMessage('SEO2_REINDEX_RESULT_TITLE_END'): GetMessage('SEO2_REINDEX_RESULT_TITLE'),
        	'TYPE' => 'OK',
        	'DETAILS' => GetMessage('SEO2_REINDEX_RESULT', array('#CNT#'=>$urlCnt, '#LAST#'=>$url, '#PAGE#'=>$page)),
        	'HTML' => true
        ));
    echo $message->Show();
...
$aTabs = array(
    array("DIV" => "edit1", "TAB" => GetMessage('SEO2_CHECK_INDEX'), "ICON" => ""),
);
...
<tr>
    <td class="field-name"><? echo GetMessage('SEO2_SITE'); ?></td>
    <td>
        <?echo SelectBoxFromArray("seo2_site_id", $arSiteDropdown);?>
    </td>
</tr>
<tr>
    <td class="field-name"><? echo GetMessage('SEO2_PAGE_N'); ?></td>
    <td>
        <input name="page" id="seo2_page" value="<? echo $pageN; ?>">
    </td>
</tr>
<tr>
    <td class="field-name"><? echo GetMessage('SEO2_SLEEP'); ?></td>
    <td>
        <? echo GetMessage('SEO2_SLEEP_MIN'); ?>&nbsp;&nbsp;<input name="sleep_min" id="seo2_sleep_min" value="5"><br/>
        <? echo GetMessage('SEO2_SLEEP_MAX'); ?>&nbsp;<input name="sleep_max" id="seo2_sleep_max" value="15">
    </td>
</tr>
<tr>
    <td class="field-name"><? echo GetMessage('SEO2_SE'); ?></td>
    <td>
        <select name="searchengine" id="searchengine">
            <option value="yandex"><? echo GetMessage('SEO2_YANDEX'); ?></option>
        </select>
    </td>
</tr>
...
<!--<input type="submit" <?if ($MODULE_RIGHT<"W") echo "disabled" ?> name="Update" value="<?echo GetMessage("MAIN_SAVE")?>">-->
    ...
    <input type="button" id="start_button" value="<? echo GetMessage('SEO2_REINDEX'); ?>" onclick="StartReindex();">
	<input type="button" id="stop_button" value="<? echo GetMessage('SEO2_REINDEX_STOP'); ?>" onclick="StopReindex();" disabled="">
	<input type="button" id="continue_button" value="<? echo GetMessage('SEO2_REINDEX_CONTINUE'); ?>" onclick="ContinueReindex();" disabled="">
<?=bitrix_sessid_post();?>
<?$tabControl->End();?>
</form>

<? echo BeginNote(); ?>
<? echo GetMessage("SEO2_DESCR") ?>
<? echo EndNote(); ?>
...
function DoNext(NS)
{
	var queryString = 'Reindex=Y'
		+ '&lang=ru';
    queryString += '&NS[sleep_min]=' + document.getElementById('seo2_sleep_min').value;
    queryString += '&NS[sleep_max]=' + document.getElementById('seo2_sleep_max').value;
    queryString += '&NS[site_id]=' + document.getElementById('seo2_site_id').value;
	...
	if(!NS)
	{
        ...
        queryString += '&NS[page]=' + document.getElementById('seo2_page').value;
