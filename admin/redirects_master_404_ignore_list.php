...
if (($arID = $lAdmin->GroupAction()) && $isAdmin) {
	if ($_REQUEST['action_target'] == 'selected') {
		$arID = Array();
		$dbResultList = seo2Redirects404IgnoreDB::GetList($arFilter);
	...
	foreach ($arID as $ID) {
		...
		switch ($_REQUEST['action']) {
			case "delete":
				@set_time_limit(0);
				seo2Redirects404IgnoreDB::Delete($ID);
				break;
		}
	}
}
...
$arResultList = seo2Redirects404IgnoreDB::GetList($arFilter, array($by => $order));
