...
if (($arID = $lAdmin->GroupAction()) && $isAdmin) {
	if ($_REQUEST['action_target'] == 'selected') {
		$arID = Array();
		$dbResultList = seo2Redirects404DB::GetList($arFilter);
...
$arResultList = seo2Redirects404DB::GetList($arFilter, array($by => $order), true);
...
$lAdminHeader = array(
	...
	array("id" => "REFERER_URL", "content" => GetMessage('SEO2_REFERER_URL'), "sort" => "REFERER_URL", "default" => true),
    //array("id" => "REDIRECT_STATUS", "content" => GetMessage('SEO2_REDIRECT_STATUS'), "sort" => "REDIRECT_STATUS", "default" => true),
	array("id" => "DATE_TIME_CREATE", "content" => GetMessage('SEO2_DATE_TIME_GENERATE'), "sort" => "DATE_TIME_CREATE", "default" => true),
);
// show guest if statistic module is enabled
if($statisticIsAvalible) $lAdminHeader[] = array("id" => "GUEST_ID", "content" => GetMessage('SEO2_GUEST'), 'default'=>true);
...
while ($arResult = $dbResultList->NavNext(true, "f_")) {
    $createRuleURL = "collected_redirects_edit.php?ADD=Y&lang=".urlencode(LANG)."&site_id=$f_SITE_ID&OLD_LINK=".urlencode($f_URL);
    $ignoreURL = "collected_redirects_404_ignore_edit.php?ADD=Y&lang=".urlencode(LANG)."&site_id=$f_SITE_ID&OLD_LINK=".urlencode($f_URL);
    ...
    // guest
    $guestUrl = ($statisticIsAvalible && $f_GUEST_ID)? '<a href="#" class="popupitem" onclick="javascript:CloseWaitWindow(); jsUtils.OpenWindow(\'guest_detail.php?lang=ru&amp;ID='.$f_GUEST_ID.'\', \'700\', \'550\'); return false;">'.GetMessage('SEO2_GUEST').'</a>': '';
	...
	if ($isAdmin)
		$arActions[] = array("ICON" => "delete", "TEXT" => GetMessage("MAIN_DELETE"), "ACTION" => "if(confirm('" . GetMessage("SEO2_DELETE_MESS") . "')) " . $lAdmin->ActionDoGroup(UrlEncode($arResult["ID"]), "delete"));
...
$aContext = array(
		array(
				"TEXT" => GetMessage("SEO2_REPORT"),
				"TITLE" => GetMessage("SEO2_REPORT_TITLE"),
				"ICON" => "",
				"LINK" => 'collected_redirects_404_report.php?lang='.urlencode(LANG)
		),
);
...
$oFilter = new CAdminFilter(
								$sTableID . "_filter",
								array(
										GetMessage('SEO2_REFERER_URL'),
                                        //GetMessage('SEO2_REDIRECT_STATUS'),
                                        GetMessage('SEO2_SITE'),
                                        GetMessage('SEO2_IS_REDRECTED_EXISTS'),
                                        GetMessage('SEO2_DATE_TIME_GENERATE'),
								)
);
...
?>
	...
	<tr>
		<td><?= GetMessage('SEO2_REFERER_URL') ?>:</td>
		<td align="left" nowrap>
    ...
    <tr>
        <td><?echo GetMessage("SEO2_DATE_TIME_GENERATE")." (".FORMAT_DATE."):"?></td>
