...
// @todo HELP_FILE
//define("HELP_FILE", "settings/seo2_redirect_edit.php");
...
if (strlen($ADD) > 0 && StrLen($site_id) <= 0 || (strlen($Update) > 0 && !$_REQUEST['ID']))
	LocalRedirect("/bitrix/admin/collected_redirects_list.php?lang=" . LANG);
...
if (strlen($ADD) > 0) {
    $sameUrl = seo2RedirectsRulesDB::GetList(array(
        'OLD_LINK'=>$OLD_LINK, 
...
if($REQUEST_METHOD == "POST") {
    ...
    elseif(strlen($ADD) > 0 && count($sameUrl)>0) {    
        $message = new CAdminMessage(array(
            'MESSAGE' => GetMessage('SEO2_ERROR_DUPLICATE_OLD_LINK'),
        	'TYPE' => 'ERROR',
        	'DETAILS' => '',
        	'HTML' => true
        ));
...
// check demo
if(strlen($ADD) > 0 && (CModule::IncludeModuleEx($sModuleId)==MODULE_DEMO or CModule::IncludeModuleEx($sModuleId)==MODULE_DEMO_EXPIRED) && seo2RedirectsRulesDB::GetCountRules() >= 5) {
    $message = new CAdminMessage(array(
...
if ($REQUEST_METHOD == "POST" && strlen($Update) > 0 && $isAdmin && check_bitrix_sessid()) {
	if (!$message) {
        ...
        $WI = ($WITH_INCLUDES != '') ? 'Y': 'N';
        $UR = ($USE_REGEXP != '') ? 'Y': 'N';
		$rr = seo2RedirectsRulesDB::Update($_REQUEST['ID'],
										array(
												"OLD_LINK" => trim($OLD_LINK),
												"NEW_LINK" => trim($NEW_LINK),
...
// find redirect by ID
$arResultList = seo2RedirectsRulesDB::GetList(array("ID" => trim($ID)/* ,'ADRESS'=>' ','DATE_TIME_CREATE'=>' ','ACTIVE'=>' ', 'COMMENT'=>' ' */));
if (count($arResultList) <= 0) {
	...
	$str_OLD_LINK = ($bOLD_LINK)? $OLD_LINK: htmlspecialchars($arResult["OLD_LINK"]);
	$str_NEW_LINK = ($bNEW_LINK)? $NEW_LINK: htmlspecialchars($arResult["NEW_LINK"]);
	$str_ACTIVE = htmlspecialchars($arResult["ACTIVE"]);
    $str_WITH_INCLUDES = htmlspecialchars($arResult["WITH_INCLUDES"]);
    $str_USE_REGEXP = htmlspecialchars($arResult["USE_REGEXP"]);
	$str_STATUS = htmlspecialchars($arResult["STATUS"]);
	$str_COMMENT = htmlspecialchars($arResult["COMMENT"]);
	$str_DATE_CREATE = htmlspecialchars($arResult["DATE_CREATE"]);
    $site_id = ($arResult["SITE_ID"])? $arResult["SITE_ID"]: $_REQUEST['site_id'];
}
//var_dump($WITH_INCLUDES);
if ($bVarsFromForm) {
	$str_OLD_LINK = htmlspecialchars($str_OLD_LINK);
	$str_NEW_LINK = htmlspecialchars($str_NEW_LINK);
	$str_ACTIVE = ($ACTIVE)? Y: N;
    $str_WITH_INCLUDES = ($WITH_INCLUDES)? Y: N;
    $str_USE_REGEXP = ($USE_REGEXP)? Y: N;
	$str_COMMENT = htmlspecialchars($COMMENT);
	$str_STATUS = htmlspecialchars($STATUS);
	$str_DATE_CREATE = htmlspecialchars($DATE_CREATE);
}
?>
...
$aMenu = array(array(
	...
	"LINK" => "/bitrix/admin/collected_redirects_list.php?lang=" . LANG . "&filter_site_id=" . UrlEncode($site_id) . "&" . GetFilterParams("filter_", false),
	"ICON" => "btn_list",
...
if(StrLen($Update) > 0) {
	...
	$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('" . GetMessage("MURL_ACT_DEL_CONF") . "')) window.location='/bitrix/admin/collected_redirects_list.php?ID=" . urlencode(urlencode($GENERATE_LINK)) . "&filter_site_id=" . urlencode($site_id) . "&action=delete&lang=" . LANG . "&" . bitrix_sessid_get() . "';",
			"WARNING" => "Y",
			"ICON" => "btn_delete"
	);
}
...
<form method="POST" action="<?= $APPLICATION->GetCurPage() ?>" name="form1">
	...
	<tr>
		...
		<td>
            ...
            <label id="seo2-redirects-edit-with-includes">
                <input type="checkbox" name="WITH_INCLUDES" <? if ($str_WITH_INCLUDES == 'Y'): ?><?= 'checked' ?><? endif ?>>
                <?= GetMessage("SEO2_WITH_INCLUDES") ?>
                <img src="/bitrix/js/main/core/images/hint.gif" onmouseover="BX.hint(this, '<?= GetMessage("SEO2_WITH_INCLUDES_TIP") ?>');">
            </label>
        </td>
	</tr>

    <tr id="seo2-redirects-edit-tr-new-link">
		<td></span><?= GetMessage('MURL_NEW_LINK') ?>:</td>
		<td><input type="text" name="NEW_LINK" id="seo2-redirects-edit-new-link" size="50" maxlength="250" value="<?= $str_NEW_LINK ?>"></td>
	</tr>
	...
	<tr>
		...
		<td>
			<select name="STATUS" id="seo2-redirects-edit-status">
	...
	<tr>
		...
		<td>
			<textarea cols="50" rows="10" name="COMMENT" ><? echo htmlspecialchars($str_COMMENT); ?></textarea>
		</td>
	</tr>
    
    <tr>
		<td><?= GetMessage("SEO2_USE_REGEXP") ?>:<br/><?= GetMessage("SEO2_USE_REGEXP_DESC") ?></td>
		<td><input type="checkbox" name="USE_REGEXP" id="seo2-redirects-edit-use-regexp" <? if ($str_USE_REGEXP == 'Y'): ?><?= 'checked' ?><? endif ?>></td>
	</tr>
	...
	$tabControl->Buttons(
        array(
            "disabled" => !$isAdmin,
			"back_url" => "/bitrix/admin/collected_redirects_list.php?lang=" . LANG . "&filter_site_id=" . UrlEncode($site_id) . "&" . GetFilterParams("filter_", false)
        )
    );
...
<script>
BX.ready(function() {
    var status = BX('seo2-redirects-edit-status');
    BX.bind(status, 'change', BX.proxy(function(e) {
        //console.log(e.target.value);
        var newLink = BX('seo2-redirects-edit-tr-new-link');
        if(e.target.value=='410') {
            BX('seo2-redirects-edit-new-link').value = '';
            BX.hide(newLink);
        }
        else BX.show(newLink);
    }, this));
    //BX('seo2-redirects-edit-status')
    BX.fireEvent(status, 'change');
    
    var useRegexp = $('#seo2-redirects-edit-use-regexp'),
        withInclude = $('#seo2-redirects-edit-with-includes');
